...
int main(int argc, char *argv[]) {
    ...
    if (write(f, header, strlen(header)) == -1) {
        ("Erreur écriture en-tête du script\n");
        exit(1);
    }
    ...
    if (argc > 1) { // verifier si on a bien recu un argument
    ...
    do {
        /* // sans ces lignes on a d'office minimum 2 commandes, on peut décommenter pour demander au user dès la première commande
        if (argc <= 1 || ch != 'N' && ch != 'n') {
        ...
        */ // Jusque ici 
        ...
        if (p == -1) {
            ...
            close(pipefd[1]);
            //execlp("/bin/sh", "/bin/sh", "-c", cmd, NULL);   // Commenté pour qu'il ny ait pas de conflit lors de l'appel du .sh après
            //perror("exec");
            exit(1);
        } else {
            // process père
            ...
            if (write(f, cmd, strlen(cmd)) == -1 || write(f, "\n", 1) == -1) { // écriture dans le .sh + vérification erreur
                fprintf(stderr, "Erreur écriture commande dans le fichier\n");
                exit(1);
            }
            c++;
        }
